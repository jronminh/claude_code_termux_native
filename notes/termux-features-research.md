...
**Re-tested again, twice, from a different session (2026-09-17, working in `claude-token-optimization`) — still inconclusive live, doc table above should be trusted instead:** that session's user pushed back on the "doesn't render" conclusion and asked for an independent re-test. Attempt 1: isolated temp `PreToolUse` hook on `Bash` returning `systemMessage` + `permissionDecision: "ask"` + `permissionDecisionReason` together — a real approve/deny UI appeared, initially read as proof `systemMessage` renders to the human. Attempt 2, tighter: same setup but with two *different* marker strings, one in `systemMessage` and one in `permissionDecisionReason`, specifically to see which one the UI actually shows — the user confirmed a confirmation-style UI appeared but **could not tell which of the two strings it contained**. Most likely explanation: the UI is the standard ask-for-permission dialog that `permissionDecision: ask` always produces (showing the reason), independent of `systemMessage` entirely — but this wasn't nailed down with certainty either. **Don't re-attempt this live-test a third time**; the doc table above (verbatim, high confidence) already settles it — `systemMessage` targets Claude, not the human — treat that as the answer instead of trying to re-observe it on-screen again.
